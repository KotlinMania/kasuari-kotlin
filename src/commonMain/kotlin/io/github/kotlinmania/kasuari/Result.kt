...
sealed class Result<out T, out E> {
     ...
     */
    inline fun unwrapOrElse(f: (E) -> @UnsafeVariance T): T =
        when (this) {
            is Ok -> value
            is Err -> f(error)
        }
     ...
     */
    inline fun <U> map(f: (T) -> U): Result<U, E> =
        when (this) {
            is Ok -> Ok(f(value))
            is Err -> Err(error)
        }
     ...
     */
    inline fun <F> mapErr(f: (E) -> F): Result<T, F> =
        when (this) {
            is Ok -> Ok(value)
            is Err -> Err(f(error))
        }
     ...
     */
    inline fun <U> andThen(f: (T) -> Result<U, @UnsafeVariance E>): Result<U, E> =
        when (this) {
            is Ok -> f(value)
            is Err -> Err(error)
        }
