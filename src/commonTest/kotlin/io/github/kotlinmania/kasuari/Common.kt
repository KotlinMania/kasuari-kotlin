// port-lint: tests tests/common/mod.rs
 ...
 */
class Values {
    private val underlyingValues: MutableMap<Variable, Double> = mutableMapOf()

    fun valueOf(variable: Variable): Double {
        val current = underlyingValues[variable]
        return current ?: 0.0
    }
    ...
    fun updateValues(changes: List<Pair<Variable, Double>>) {
        for ((variable, value) in changes) {
            ...
            underlyingValues[variable] = value
        }
    }
}
